---
title: "Guía de Extensión y Creación de Plugins: callahan"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de callahan"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-callahan-plugins)=
# Guía de Extensión y Plugins: callahan

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`callahan`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-callahan-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`callahan` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para callahan`.
- **Punto de Entrada Oficial**: `callahan.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-callahan-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para callahan."""
    name = "custom_rule"
...
Para que `callahan` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."callahan.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `callahan` la reconozca:
...
callahan plugins list
````

---

(manual-callahan-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `callahan` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = callahan.ejecutar_analisis(
...
(manual-callahan-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `callahan` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar callahan
        run: pip install -e ./callahan
        
      - name: Ejecutar Auditoría
        run: callahan check src/ include/ --json > reporte.json
````

---

(manual-callahan-plugins-ejercicios)=
