...
### 2.2 Métodos de Instalación de `callahan`

Podés instalar `callahan` mediante cualquiera de los siguientes métodos estándar:
...
La interfaz CLI de `callahan` cuenta con autocompletado nativo para comandos, flags y archivos. Para configurarlo permanentemente en tu shell:

````{code-block} bash
# Configuración automática en Bash / Zsh / Fish
callahan --install-completion
